                                     ...
                                     struct rt_i2c_msg        *msg)
{
    ...
    if (flags & RT_I2C_ADDR_10BIT)
    {
        ...
        if (flags & RT_I2C_RD)
        {
            ...
            addr1 |= 0x01;
            ret = i2c_send_address(bus, addr1, retries);
            // LOG_W("%s,L:%d,add1:0x%0X,ret:%d\n",__func__, __LINE__, addr1, ret);
            if ((ret != 1) && !ignore_nack)
            {
                              ...
                              rt_uint32_t               num)
{
    ...
    for (i = 0; i < num; i++)
    {
        ...
        if (!(msg->flags & RT_I2C_NO_START))
        {
            ...
            // LOG_W("%s,L:%d,ret:%d\n",__func__, __LINE__, ret);
            // LOG_I("msg[%d].addr:0x%0X",i, msg->addr);
            // LOG_I("msg[%d].flags:0x%0X",i, msg->flags);
            // LOG_I("msg[%d].len:0x%0X",i, msg->len);
